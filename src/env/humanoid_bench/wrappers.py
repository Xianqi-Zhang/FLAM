...
class DoubleReachBaseWrapper(BaseWrapper):
    # action is [left_hand_target, phi, costheta, u],
    # where (phi, costheta, u) are the spherical coordinates of the
    # right hand target wrt the left hand target
...
class ObservationWrapper(BaseWrapper):
    ...
    def observation_space(self):
        camera_orient_space = Box(low=-np.inf, high=np.inf, shape=(1, 9), dtype=np.float64)
        camera_pos_space = Box(low=-np.inf, high=np.inf, shape=(1, 3), dtype=np.float64)
        proprio_space_shape = (self.task._env.robot.dof * 2 - 1 - 13,)
        proprio_space = Box(low=-np.inf, high=np.inf, shape=proprio_space_shape, dtype=np.float64)
        global_pos_space = Box(low=-np.inf, high=np.inf, shape=(23, 3), dtype=np.float64)
        global_orient_space = Box(low=-np.inf, high=np.inf, shape=(23, 4), dtype=np.float64)

        # * Since I want to use camera pose and robot pose, but w/o
        # * other obs, the following code is commented out.
        # if not self._privileged_ob and not self._tactile_ob and not self._camera_ob:
        #     return proprio_space

        spaces = [
            ("camera_orient", camera_orient_space),
            ("camera_pos", camera_pos_space),
            ("proprio", proprio_space),
            ('global_orient', global_orient_space),
            ('transl', global_pos_space),
        ]

        if self._camera_ob:
            image_obs = self.get_camera_obs()
            camera_spaces = [
                (
                    key,
                    Box(low=0, high=255, shape=image_obs[key].shape, dtype=np.uint8),
                )
        ...
        if self._tactile_ob:
            ...
            tactile_spaces = [
                (
                    key,
                    Box(low=-np.inf, high=np.inf, shape=tactile_obs[key].shape, dtype=np.float64)
                )
                for key in tactile_obs
            ]
    ...
    def get_obs(self):
        ...
        # * Since I want to use camera pose and robot global pose, but
        # * w/o other obs, the following code is commented out.
        # if not self._privileged_ob and not self._tactile_ob and not self._camera_ob:
        #     return state
        ...
        # * Add camera orientation and position.
        camera_orinet, camera_pos = self.task._env.robot.camera_pose()
        obses.append(("camera_orient", camera_orinet))
        obses.append(("camera_pos", camera_pos))
        # * Add global position and orientation.
        obses.append(("transl", self.task._env.robot.global_position()))
        obses.append(("global_orient", self.task._env.robot.global_orientation()))

        return dict(obses)

    def get_tactile_obs(self):
        """
        Touch data is in the form of a dictionary with keys as the
        sensor names and values as the touch data.
        The touch data is then reshaped to a 3D array with the first
        dimension as the number of components (x-y-z),
        and the second and third dimensions as the touch data in a 2D
        grid, e.g., np.reshape(touch_data, (3, 2, 4))[[1, 2, 0]]
