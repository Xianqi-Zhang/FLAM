...
from src.policy.scale import RunningScale
from src.policy.world_model import WorldModel
...
class TDMPC2():
    ...
    def __init__(self, cfg):
        ...
        self.enc_lr = self.cfg.lr * self.cfg.enc_lr_scale
        self.optim = torch.optim.Adam(
            [
                {'params': self.model._encoder.parameters(), 'lr': self.enc_lr},
    ...
    def act(self, obs, t0=False, eval_mode=False, task=None):
        ...
        if isinstance(obs, dict):
            if self.cfg.obs in obs:
                # * collections.OrderedDic
                # * keys: ['camera_orient', 'camera_pos', 'proprio', 'global_orient', 'transl']
                obs = torch.Tensor(obs[self.cfg.obs]).float()
                obs = obs.squeeze(0)
            else:
                raise ValueError('Proprioception data not found in observation dictionary')
    ...
    def update(self, buffer):
        ...
        Args:
            buffer (utils.buffer.Buffer): Replay buffer.
        Returns:
        ...
        for t in range(self.cfg.horizon):
            coef = self.cfg.rho ** t
            reward_loss += math.soft_ce(reward_preds[t], reward[t], self.cfg).mean() * coef
            for q in range(self.cfg.num_q):
                value_loss += math.soft_ce(qs[q][t], td_targets[t], self.cfg).mean() * coef
