"""
Code:
	- https://github.com/nicklashansen/tdmpc2/blob/vectorized_env/tdmpc2/common/buffer.py
...
class Buffer():
    ...
    def _init(self, tds):
        ...
        bytes_per_step = sum([
            (
                ...
                if not isinstance(v, TensorDict) else sum(
                    [x.numel() * x.element_size() for x in v.values()]
                )
            ) for v in tds.values()
        ]) / len(tds)
    ...
    def _prepare_batch(self, td):
        ...
        - https://github.com/nicklashansen/tdmpc2/issues/34
        """
        obs = td['obs'].float()
        action = td['action'][1:]
