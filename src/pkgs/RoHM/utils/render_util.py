...
material_debug = pyrender.MetallicRoughnessMaterial(
    metallicFactor=0.0,
    alphaMode='OPAQUE',
    baseColorFactor=(1.0, 0.0, 0.0, 1.0)  # * Red.
)
material_skel_debug = pyrender.MetallicRoughnessMaterial(
    metallicFactor=0.0,
    alphaMode='OPAQUE',
    baseColorFactor=(0.0, 0.0, 0.0, 1.0)  # * Black.
)

def create_render_cam(cam_x, cam_y, fx=1060.53, fy=1060.38):
    # H, W = 1080, 1920
...
def create_pyrender_skel(
    ...
    for index_pair in LIMBS_BODY_SMPL:
        ...
        if mask_scheme in ['lower', 'upper']:
            ...
            material = material_skel_vis if in_mask_area else material_joint_occ
        if not (mask_scheme in ['lower', 'upper'] and not add_occ_joints and (index_pair[0] in mask_joint_id or index_pair[1] in mask_joint_id)):
