...
from src.pkgs.RoHM.diffusion.fp16_util import MixedPrecisionTrainer
from src.pkgs.RoHM.diffusion.resample import create_named_schedule_sampler
        ...
        for epoch in range(self.num_epochs):
            ...
            for batch in tqdm(self.train_dataloader):
                ...
                if not self.input_noise:
                    batch['pose_cond'] = batch['motion_repr_clean'].clone()  # [bs, clip_len, body_feat_dim]
                else:
                    batch['pose_cond'] = batch['motion_repr_noisy'].clone()
                ...
                if epoch <= self.start_prox_mask_epoch:
                    ...
                    for i in range(bs):
                        for k in range(3):
                            batch['pose_cond'][i, :, traj_feat_dim + mask_joint_id[i] * 3 + k] = 0.
                        for k in range(3):
                            batch['pose_cond'][i, :, traj_feat_dim + 22 * 3 + mask_joint_id[i] * 3 + k] = 0.
                        for k in range(6):
                            batch['pose_cond'][i, :, traj_feat_dim + 22 * 3 + 22 * 3 + (mask_joint_id[i] - 1) * 6 + k] = 0.
                        if 7 in mask_joint_id[i] or 10 in mask_joint_id[i]:  # left foot
                            batch['pose_cond'][i, :, -4:-2] = 0.
                        if 8 in mask_joint_id[i] or 11 in mask_joint_id[i]:  # right foot
                            batch['pose_cond'][i, :, -2:] = 0.
                ...
                # lower / upper / full: mask out all lower/upper/full body joints
                    ...
                    if 'prox' in prob_dict.keys() and prob <= prob_dict['prox']:
                        ...
                        batch['pose_cond'] = batch['pose_cond'] * prox_mask
                    #### mask out lower body part
                    elif 'lower' in prob_dict.keys() and prob <= prob_dict['lower']:
                        lower_joint_id = np.asarray([1, 2, 4, 5, 7, 8, 10, 11])
                        for k in range(3):
                            batch['pose_cond'][:, :, traj_feat_dim + lower_joint_id * 3 + k] = 0.
                        for k in range(3):
                            batch['pose_cond'][:, :, traj_feat_dim + 22 * 3 + lower_joint_id * 3 + k] = 0.
                        for k in range(6):
                            batch['pose_cond'][:, :, traj_feat_dim + 22 * 3 + 22 * 3 + (lower_joint_id - 1) * 6 + k] = 0.
                        batch['pose_cond'][:, :, -4:] = 0.
                    #### mask out upper body part
                        ...
                        for k in range(3):
                            batch['pose_cond'][:, :, traj_feat_dim + upper_joint_id_select * 3 + k] = 0.
                        for k in range(3):
                            batch['pose_cond'][:, :, traj_feat_dim + 22 * 3 + upper_joint_id_select * 3 + k] = 0.
                        for k in range(6):
                            batch['pose_cond'][:, :, traj_feat_dim + 22 * 3 + 22 * 3 + (upper_joint_id_select - 1) * 6 + k] = 0.
                        batch['pose_cond'][:, :, -4:] = 0.
                    #### mask out full body of a sub-sequence
                        ...
                        batch['pose_cond'][:, :, -4:] = 0.
                        for idx in range(bs):
                            batch['pose_cond'][idx, start[idx]:end[idx], traj_feat_dim:] = 0  # do not mask out traj part
                if self.input_noise:
                    batch['pose_cond'][:, :, -4:] = 0.

                batch['motion_repr_clean'] = torch.permute(batch['motion_repr_clean'], (0, 2, 1)).unsqueeze(-2)  # [bs, body_feat_dim, 1, clip_len]
                batch['pose_cond'] = torch.permute(batch['pose_cond'], (0, 2, 1)).unsqueeze(-2)
                ...
                if self.step % self.log_interval == 0 and self.step > 0:
                    ...
                    for test_step, test_batch in tqdm(enumerate(self.test_dataloader)):
                        ...
                        if not self.input_noise:
                            test_batch['pose_cond'] = test_batch['motion_repr_clean'].clone()  # [bs, clip_len, 263]
                        else:
                            test_batch['pose_cond'] = test_batch['motion_repr_noisy'].clone()
                        ...
                        for i in range(bs):
                            for k in range(3):
                                test_batch['pose_cond'][i, :, traj_feat_dim + mask_joint_id[i] * 3 + k] = 0.
                            for k in range(3):
                                test_batch['pose_cond'][i, :, traj_feat_dim + 22 * 3 + mask_joint_id[i] * 3 + k] = 0.
                            for k in range(6):
                                test_batch['pose_cond'][i, :,
                                traj_feat_dim + 22 * 3 + 22 * 3 + (mask_joint_id[i] - 1) * 6 + k] = 0.
                            if 7 in mask_joint_id[i] or 10 in mask_joint_id[i]:  # left foot
                                test_batch['pose_cond'][i, :, -4:-2] = 0.
                            if 8 in mask_joint_id[i] or 11 in mask_joint_id[i]:  # right foot
                                test_batch['pose_cond'][i, :, -2:] = 0.
                        if self.input_noise:
                            test_batch['pose_cond'][:, :, -4:] = 0.

                        test_batch['motion_repr_clean'] = torch.permute(test_batch['motion_repr_clean'], (0, 2, 1)).unsqueeze(-2)  # [bs, body_feat_dim, 1, clip_len]
                        test_batch['pose_cond'] = torch.permute(test_batch['pose_cond'], (0, 2, 1)).unsqueeze(-2)
