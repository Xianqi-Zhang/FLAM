    ...
    if process:
        ...
        with torch.no_grad():
            for fId in range(0, frame_number, down_sample):
                ...
                smplx_params_item = torch.cat([global_orient, trans, betas, body_pose, hand_pose,
                                               jaw_pose, leye_pose, reye_pose], dim=-1)
                smplx_params_seq.append(smplx_params_item)  # [1, 178]
                pose_seq.append(smplx_joints)

        pose_seq = torch.cat(pose_seq, dim=0)
        pose_seq_np = pose_seq.detach().cpu().numpy()  # [seq_len, 52, 3], position of 52 body joints?

        smplx_params_seq = torch.cat(smplx_params_seq, dim=0)  # [seq_len, 178]
        smplx_params_seq_np = smplx_params_seq.detach().cpu().numpy()  # [seq_len, 178],
...
if __name__ == "__main__":
    ...
    parser.add_argument('--amass_root', type=str, default='./datasets', help='Root dir of raw AMASS data (smplx neutral body)')
    # parser.add_argument('--dataset_name', type=str, default='ACCAD', help='AMASS subset name')
    parser.add_argument('--dataset_name', type=str, default='ToTalCapture',
                        choices=['ACCAD', 'TCDHands', 'TotalCapture', 'SFU'],
                        help='AMASS subset name')
    # AMASS_preprocessed
    parser.add_argument('--save_root', type=str, default='./datasets/AMASS_smplx_preprocessed', help='Root directory to save preprocessed data to.')
