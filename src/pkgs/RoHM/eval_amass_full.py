...
group.add_argument('--saved_data_path', type=str,
                   default='test_results/results_amass_full/test_amass_full_grad_True_mask_full_noise_3_infill_traj_0.1_iter_2_iter2trajnoisy_False_iter2posenoisy_True_earlystop_False_seed_0.pkl',
    ...
    if args.render:
        ...
        for bs in range(0, n_seq, args.render_interval):
            ...
            for t in range(0, clip_len, 1):
                ...
                ################## add body skeleton
                in_full_mask_area = (args.mask_scheme == 'full') and (t < start or t >= end)
                skeleton_mesh_rec_list = create_pyrender_skel(joints=rec_ric_data_rec_list_from_smpl[bs, t], add_trans=np.linalg.inv(cam_trans),
                                                              ...
                                                              in_mask_area=in_full_mask_area, contact_lbl=contact_lbl_rec_list[bs, t])
                skeleton_mesh_noisy_list = create_pyrender_skel(joints=rec_ric_data_noisy_list[bs, t], add_trans=np.linalg.inv(cam_trans), mask_scheme=args.mask_scheme,
                                                                mask_joint_id=cur_mask_joint_id, add_occ_joints=False, in_mask_area=in_full_mask_area)
