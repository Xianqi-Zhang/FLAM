...
class ControlNet(nn.Module):
    ...
    def forward(self, batch, h_cond, time_embed):
        '''
            batch['traj_control_cond'] : [bs, T, control_feat_dim]
            time_embed: [bs, 32], diffusion timestep embedding
        '''
        control_cond = batch['traj_control_cond']
...
class TrajNet(nn.Module):
    ...
    def __init__(
            self,
            time_dim=32,
            cond_dim=13,  # * traj_dataset.traj_feat_dim
            mid_dim=512,
            traj_feat_dim=13,  # * traj_dataset.traj_feat_dim
            repr_abs_only=False,
            std=None,
            mean=None,
            device=None,
            ...
            weight_loss_root_pos_global=0.0,
            weight_loss_root_vel_global=0.0,
            weight_loss_root_rot_vel_from_abs_traj=0.0,
            weight_loss_root_smplx_transl_vel=0.0,
            weight_loss_root_smplx_rot_vel=0.0,
        ...
        if std is None:
            std_file_path = 'src/pkgs/RoHM/data/checkpoints/trajnet_checkpoint/AMASS_std.pkl'
            ...
            self.std = std
        if mean is None:
            mean_file_path = 'src/pkgs/RoHM/data/checkpoints/trajnet_checkpoint/AMASS_mean.pkl'
    ...
    def compute_losses_with_smpl(self, batch, model_output, smplx_model=None):
        ...
        full_repr_clean = batch['motion_repr_clean'] * torch.from_numpy(self.std).to(self.device) + torch.from_numpy(self.mean).to(self.device)
