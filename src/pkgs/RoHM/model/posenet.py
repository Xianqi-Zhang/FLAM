...
import smplx
import pickle
from src.pkgs.RoHM.model.heads import *
...
class PoseNet(nn.Module):
    def __init__(
            ...
            weight_loss_joint_vel_global=0.0,
        ...
        if std is None:
            std_file_path = 'src/pkgs/RoHM/data/checkpoints/posenet_checkpoint/AMASS_std.pkl'
            with open(std_file_path, 'rb') as f:
                data_std = pickle.load(f)
                data_std = np.concatenate([data_std[key] for key in data_std.keys()], axis=-1)
                self.std = torch.from_numpy(data_std).to(device)
        else:
            self.std = std
        if mean is None:
            mean_file_path = 'src/pkgs/RoHM/data/checkpoints/posenet_checkpoint/AMASS_mean.pkl'
            with open(mean_file_path, 'rb') as f:
                data_mean = pickle.load(f)
                data_mean = np.concatenate([data_mean[key] for key in data_mean.keys()], axis=-1)
                self.mean = torch.from_numpy(data_mean).to(device)
        else:
            self.mean = mean
        ...
        self.output_process = OutputProcess(self.pose_feat_dim, self.latent_dim, self.nfeats)
    ...
    def forward(self, batch, timesteps):
        ...
        cond = self.input_process_cond(batch['pose_cond'])
        x = x + cond
        ...
        given_traj = batch['pose_cond'][:, 0:self.traj_feat_dim, :]  # gt traj for training, trajnet output for test
    ...
    def compute_losses_with_smpl(self, batch, model_output, smplx_model=None, epoch=0):
        ...
        full_repr_clean = full_repr_clean * torch.from_numpy(self.std).to(self.device) + torch.from_numpy(self.mean).to(self.device)
        ...
        full_repr_rec = full_repr_rec * torch.from_numpy(self.std).to(self.device) + torch.from_numpy(self.mean).to(self.device)
        # reconstruct joint positions
        cur_total_dim = 0
        repr_dict_rec = {}
        for repr_name in REPR_LIST:
    ...
    def guide_skating_with_smpl(self, batch, out, denoise_t, compute_grad='x_t'):
        ...
        with torch.enable_grad():
            ...
            # reconstruct joint positions
            traj_feat_dim = self.traj_feat_dim
            cur_total_dim = 0
            repr_dict_rec = {}
            for repr_name in REPR_LIST:
                repr_dict_rec[repr_name] = full_repr_rec[..., cur_total_dim:(cur_total_dim + REPR_DIM_DICT[repr_name])]
    ...
    def guide_2d_projection_with_smpl(self, batch, out, denoise_t, compute_grad='x_t'):
        ...
        with torch.enable_grad():
            ...
            T = T.unsqueeze(1).unsqueeze(1).repeat(1, clip_len, self.joints_num, 1).reshape(-1, self.joints_num, 3)
            ...
            joints_rec_scene_coord_from_smpl = joints_rec_scene_coord_from_smpl.reshape(batch_size, clip_len, self.joints_num, 3)

            ###### trans to camera coord
            joints_rec_scene_coord_from_smpl = joints_rec_scene_coord_from_smpl.reshape(batch_size*clip_len, -1, 3)
            joints_rec_cam_coord = torch.matmul(torch.linalg.inv(self.cam_R), (joints_rec_scene_coord_from_smpl - self.cam_t).permute(0, 2, 1)).permute(0, 2, 1)  # [batch_size*clip_len, 22, 3]
            ###### project to 2d
