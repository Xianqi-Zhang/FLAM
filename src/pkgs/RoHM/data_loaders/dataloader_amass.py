...
from src.pkgs.RoHM.data_loaders.motion_representation import *
...
class DatasetAMASS(data.Dataset):
    def __init__(
        ...
        if self.task not in ['traj', 'pose']:
            raise ValueError('[ERROR] args.traj should be in [traj, pose]!')
        ...
        if not repr_abs_only:  # * repr_abs_only = True
            self.traj_repr_name_list = ['root_rot_angle', 'root_rot_angle_vel', 'root_l_pos',
                                        'root_l_vel', 'root_height', 'smplx_rot_6d',
                                        'smplx_rot_vel', 'smplx_trans', 'smplx_trans_vel']
        else:
        ...
        self.body_feat_dim = 0  # * 294
        self.traj_feat_dim = 0  # * 22
        self.pose_feat_dim = 0  # * 272
        ...
        self.repr_list_dict = {repr_name: [] for repr_name in REPR_LIST}
        self.smplx_params_list_dict = {
            'global_orient': [], 'transl': [], 'body_pose': [], 'betas': []
        }  # each item: params of all samples

        self.joints_clean_list = []
        if self.input_noise and (not self.sep_noise):
            self.joints_noisy_list = []
            self.repr_list_dict_noisy = {repr_name: [] for repr_name in REPR_LIST}
    ...
    def divide_clip(self, dataset_name='HumanEva'):
        ...
        for path in seqs_path:
            ...
            if N >= self.clip_len:
                ...
                for i in range(num_valid_clip):  # * clip=145
    ...
    def read_data(self, amass_datasets):
        ...
        for dataset_name in pbar:
            pbar.set_description('Dataset subset {}'.format(dataset_name))
    ...
    def create_body_repr(self):
        smplx_noise_dict = {}
        pbar = tqdm(range(0, self.n_samples, self.spacing))
        for i in pbar:
            pbar.set_description('Body repr generation')
            source_data_joints = self.joints_clip_list[i][:, 0:self.joints_num, :]  # [T, 22, 3]
            source_data_smplx = self.smplx_clip_list[i]  # [T, 178]
            smplx_params_dict = {
                'global_orient': source_data_smplx[:, 0:3],  # * 3
                'transl': source_data_smplx[:, 3:6],  # * 3
                'betas': source_data_smplx[:, 6:16],  # * 10
                'body_pose': source_data_smplx[:, 16:(16 + 63)],  # * 63
            }  # for the clip
            ...
            if self.input_noise and (not self.sep_noise):
                    ...
                    if param_name == 'transl' or param_name == 'betas':
                        if self.load_noise:
                            ...
                            noise_1 = np.random.normal(
                                loc=0.0,
                                scale=self.noise_std_params_dict[param_name],
                                size=cano_smplx_params_dict[param_name].shape
                            )
                        ...
                        if self.load_noise:
                            ...
                            noise_body_pose_rot = np.random.normal(
                                loc=0.0,
                                scale=self.noise_std_params_dict[param_name],
                                size=body_pose_angle.shape
                            )
                ...
                cano_positions_noisy = self.smplx_neutral(**smplx_params_dict_noisy_torch).joints[:, :22].detach().cpu().numpy()  # [clip_len, 22, 3]
    ...
    def __getitem__(self, index):
        ...
        repr_dict_clean = {}
        for repr_name in REPR_LIST:
            repr_dict_clean[repr_name] = self.repr_list_dict[repr_name][index]  # [clip_len=144, d]
        ...
        if self.input_noise:
            if self.sep_noise:
                ...
                ######## creat noisy repr
                repr_dict_noisy = get_repr_smplx(positions=positions_noisy,
                                                 smplx_params_dict=smplx_params_dict_noisy)
            else:
        ...
        if self.task == 'traj':
            ...
            item_dict['traj_cond'] = noisy_traj  # condition of TrajNet: noisy trajectory
            item_dict['traj_control_cond'] = item_dict['motion_repr_clean'][:, -self.pose_feat_dim:]  # PoseControl signal: clean local pose features
