...
from src.pkgs.RoHM.utils.konia_transform import *
from src.pkgs.RoHM.data_loaders.common.quaternion import *
...
def foot_detect(positions, thres, up_axis='y'):
    ...
    feet_l = (((feet_l_x + feet_l_y + feet_l_z) < velfactor) & (feet_l_h < heightfactor))

    feet_r_x = (positions[1:, fid_r, 0] - positions[:-1, fid_r, 0]) ** 2
    feet_r_y = (positions[1:, fid_r, 1] - positions[:-1, fid_r, 1]) ** 2
    feet_r_z = (positions[1:, fid_r, 2] - positions[:-1, fid_r, 2]) ** 2
    feet_r_h = positions[:-1, fid_r, up_axis_dim]
    feet_r = (((feet_r_x + feet_r_y + feet_r_z) < velfactor) & (feet_r_h < heightfactor))
    return feet_l.astype(float), feet_r.astype(float)
...
def cano_seq_smplx(
    ...
    """
    ##### positions: z axis up
    cano_positions = positions.copy()  # * (bs, 22, 3)
    ...
    # * ano_positions[:, :, 2] -= floor_height  # z: up-axis, foot on ground

    # * Modified.
    if put_on_floor:
        cano_positions[:, :, 2] -= cano_positions[:, :, 2].min(axis=1).reshape(-1, 1)
    if preset_floor_height:
        floor_height = preset_floor_height
    else:
        floor_height = cano_positions[:, :, 2].min()
    ...
    # * Update global_orient and transl.
    if update_globalRT:
        ...
        cano_smplx_params_dict = smplx_params_dict
...
def get_repr_smplx(positions, smplx_params_dict, feet_vel_thre=5e-5):
    ...
    input:
        ...
        * NOTE: input shape (clip_len)
    Output:
        - data_dict: our motion representation (both traj and local pose)
        * NOTE: output shape (clip_len-1)
    """
    global_positions = positions.copy()

    """ Get Foot Contacts """
    feet_l, feet_r = foot_detect(positions, feet_vel_thre,
                                 up_axis='z')  # feet_l/feet_r: [seq_len, 2]
    ...
    root_rot_angle = np.arctan2(root_rot_quat[:, 3:4], root_rot_quat[:,
                                                       :1])  # rotation angle, is half of the actual angle...
    root_rot_angle_vel = np.arctan2(root_rot_quat_vel[:, 3:4],
                                    root_rot_quat_vel[:, :1])  # rotation angle velocity

    """local joint positions"""
    local_positions = positions.copy()  # [seq_len, 22, 3]
    local_positions[..., 0] -= local_positions[:, :1, 0]
    local_positions[..., 1] -= local_positions[:, :1, 1]
    """for each frame, local pose face y+"""
    local_positions = qrot_np(np.repeat(root_rot_quat[:, None], local_positions.shape[1], axis=1),
                              local_positions)
    ...
    dRdt = smplx_rot_mat[1:] - smplx_rot_mat[:-1]  # [seq_len-1, 3, 3]
    smplx_rot_vel = estimate_angular_velocity_np(smplx_rot_mat[:-1], dRdt)
    ...
    smplx_body_pose_mat = R.from_rotvec(smplx_body_pose_aa.reshape(-1, 3)).as_matrix()
    smplx_body_pose_mat = smplx_body_pose_mat.reshape(len(smplx_body_pose_aa), -1, 3, 3)
    smplx_body_pose_6d = smplx_body_pose_mat[..., :-1].reshape(
        [smplx_body_pose_mat.shape[0], -1, 6])

    """smpl shape"""
    smplx_betas = smplx_params_dict['betas']  # [seq_len, 10]

    ################### final full body repr #####################
    # * Since RoHM neeeds to calculate velocities and foot sliding,
    # * the input seq must more than 2 frames, i.e., clip_len >= 2.
    data_dict = {
        'root_rot_angle': root_rot_angle[:-1],
        'root_rot_angle_vel': root_rot_angle_vel,
        'root_l_pos': root_l_pos[:-1, [0, 1]],
        'root_l_vel': root_l_vel[:, [0, 1]],
        'root_height': root_height[:-1],

        'smplx_rot_6d': smplx_rot_6d[:-1],
        'smplx_rot_vel': smplx_rot_vel,
        'smplx_trans': smplx_trans[:-1],
        'smplx_trans_vel': smplx_trans_vel,

        'local_positions': local_positions[:-1].reshape(len(smplx_trans_vel), -1),
        'local_vel': local_vel.reshape(len(smplx_trans_vel), -1),

        'smplx_body_pose_6d': smplx_body_pose_6d[:-1].reshape(len(smplx_trans_vel), -1),
        'smplx_betas': smplx_betas[:-1],
...
def convert_repr_dict_to_smplx_params_dict(repr_dict):
    # * Convert to smplx_params_dict.
    global_orient_mat = rot6d_to_rotmat(repr_dict['smplx_rot_6d'].reshape(-1, 6))  # [bs*T, 3, 3]
    ...
    g_len = len(global_orient_aa)
    g_device = global_orient_aa.device
    smplx_params_dict = {
        # * Body pose.
        'transl': repr_dict['smplx_trans'].reshape(-1, 3),
        'global_orient': global_orient_aa,
        'body_pose': body_pose_aa.reshape(-1, 63),
        'betas': repr_dict['smplx_betas'].reshape(-1, 10),
        # * Not used.
        'jaw_pose': torch.zeros(g_len, 3).to(g_device),
        'leye_pose': torch.zeros(g_len, 3).to(g_device),
        'reye_pose': torch.zeros(g_len, 3).to(g_device),
        'left_hand_pose': torch.zeros(g_len, 45).to(g_device),
        'right_hand_pose': torch.zeros(g_len, 45).to(g_device),
        'expression': torch.zeros(g_len, 10).to(g_device)
    }
    return smplx_params_dict


def recover_from_repr_smpl(
        data_dict,
        smplx_model=None,
        recover_mode='smplx_params',
        return_verts=True,
    ...
    input:
        ...
        - return_full_joints: return all 127 smplx joints dimensions.
    output:
        - smplx_joints: smplx joints, only include 22 main body joints if not return_full_joints
        - smplx_verts: smplx vertices

    * smpl_verts: shape (clip_len, 10475, 3)
    * smpl_verts: keys
        - 'root_rot_angle', 'root_rot_angle_vel', 'root_l_pos', 'root_l_vel', 'root_height',
        - 'smplx_rot_6d', 'smplx_rot_vel', 'smplx_trans', 'smplx_trans_vel', 'local_positions',
        - 'local_vel', 'smplx_body_pose_6d', 'smplx_betas', 'foot_contact'
    """
    if recover_mode not in ['joint_abs_traj', 'joint_rel_traj', 'smplx_params']:
        raise NotImplementedError('[ERROR] recover_mode incorrect!')
    ...
    if recover_mode[0:5] == 'joint':
        ...
        if recover_mode == 'joint_abs_traj':
            root_traj_repr = torch.cat([root_rot_angle, root_l_pos, root_height], axis=-1)
            r_rot_quat, r_pos = recover_root_rot_pos(data=root_traj_repr, root_traj_repr='abs',
                                                     up_axis='z')
        elif recover_mode == 'joint_rel_traj':
            root_traj_repr = torch.cat([root_rot_angle_vel, root_l_vel, root_height], axis=-1)
            r_rot_quat, r_pos = recover_root_rot_pos(data=root_traj_repr, root_traj_repr='rel',
                                                     up_axis='z')
        ...
        positions = qrot(qinv(r_rot_quat[..., None, :]).expand(positions.shape[:-1] + (4,)),
                         positions)
        ...
        smplx_params_dict = convert_repr_dict_to_smplx_params_dict(data_dict)
        ...
        smplx_joints = smplx_joints.detach().cpu().numpy() if to_numpy else smplx_joints

        if return_verts:
            smplx_verts = smplx_output.vertices.reshape(bs, -1, 10475, 3)
            smplx_verts = smplx_verts.detach().cpu().numpy() if to_numpy else smplx_verts
