...
class GaussianDiffusionPoseNet:
    ...
    def p_mean_variance(self, model, batch, x, t, clip_denoised=True, denoised_fn=None,
                        model_kwargs=None):
        """
    ...
    def p_sample(
        ...
        nonzero_mask = (
            (t != 0).float().view(-1, *([1] * (len(x.shape) - 1))))  # no noise when t == 0
        # sample x_{t-1} from q(x_{t-1}|x_t, x_0)~N(x_{t-1}, mu(x_t, x_0), ...)
    ...
    def p_sample_with_grad(
        ...
        if grad_type == 'prox':
            ...
            if t[0] <= 100:
                grad_2d_projection = model.guide_2d_projection_with_smpl(batch, out, t,
                                                                         compute_grad='x_0')  # [bs, 263]
                out["mean"] = out["mean"].float() + grad_weight_2d_projection * out[
                    'variance'] * grad_2d_projection.float()
            ############ calculate cond grad foot skaing
            grad_weight_skating = 1e5  # todo 5e4 for 50
            if t[0] <= 100:  # todo
                grad_skating = model.guide_skating_with_smpl(batch, out, t,
                                                             compute_grad='x_0')  # [bs, 263]
                out["mean"] = out["mean"].float() + grad_weight_skating * out[
                    'variance'] * grad_skating.float()

        elif grad_type == 'amass':
            grad_weight_skating = 3e6  # todo 5e4 for 50
            if t[0] <= 50:  # todo
                # [bs, 263]
                grad_skating = model.guide_skating_with_smpl(batch, out, t, compute_grad='x_0')
                out["mean"] = out["mean"].float() + grad_weight_skating * out[
                    'variance'] * grad_skating.float()
    ...
    def p_sample_loop(
        ...
        if not save_intermediate_result:
            return final["pred_xstart"] if early_stop else final["sample"]
        else:
            intermediate_x0_list.append(sample['pred_xstart'].clone().detach())
            intermediate_xt_list.append(sample['x_t'].clone().detach())
            intermediate_t_list.append(self.num_timesteps - i - 1)
        ...
        for i in indices:
            ...
            with th.no_grad():
                ...
                out = sample_fn(model,
                                ...
                                clip_denoised=clip_denoised,
                                denoised_fn=denoised_fn,
                                cond_fn=cond_fn if cond_fn_with_grad else None,
                                model_kwargs=model_kwargs,
                                const_noise=const_noise)
                yield out
    ...
    def ddim_sample(
        ...
        out = self.condition_score(cond_fn, out_orig, x, t, model_kwargs=model_kwargs) \
            if cond_fn is not None else out_orig
    ...
    def ddim_sample_loop(
        ...
        """
        if (dump_steps is not None) or (const_noise == True):
    ...
    def eval_losses(
        ...
        if timestep_respacing == '':
            model_output = self.p_sample_loop(model=model.model,
                                              batch=batch,
                                              shape=shape,
                                              progress=progress,
                                              clip_denoised=clip_denoised,
